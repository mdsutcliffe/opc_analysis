...
write.table(x = signature$tpm,file = "./temp/mixture_signature_pericytes.txt",quote = F,sep = "\t",row.names = F)
write.table(x = signature$matrix,file = "./temp/signature_matrix_pericytes.txt",quote = F,sep = "\t",row.names = F)

# Run cibersort, get results, and place in external folder

f.cibersort <- "./external/CIBERSORTx_validation_pericyte.txt"
...
pdf(file = "./plots/cibersort_signature_validation_pericytes.pdf",width = 6,height = 6)
par(mar = c(1.8,4,1,8),mgp = c(2.9,1,0),xpd = T)
barplot(t(as.matrix(cibersort_signature)),ylab = "Fraction",names.arg = rep(x = "",ncol(cibersort_signature)),col = rev(brewer.pal(7,"Set1")),las = 1)
title(xlab = "Signatures as mixtures",mgp = c(0.5,0,0))
legend(x = "topright",legend = names(cibersort_signature),pch = 15,col = rev(brewer.pal(7,"Set1")),inset = c(-0.4,0))
dev.off()
...
pdf(file = "./plots/purification_bar_pericytes.pdf",width = 4,height = 3,pointsize = 7)

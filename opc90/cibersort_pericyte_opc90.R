...
write.table(x = opc90_mixture,file = "./temp/mixture_opc90_pericyte.txt",quote = F,sep = "\t",row.names = F)

# Run cibersort

f.cibersort_opc90 <- "./external/CIBERSORTx_opc90_pericyte.txt"
...
pdf(file = "./plots/cibersort_opc90_pericyte.pdf",width = 3,height = 3,pointsize = 6,useDingbats = F)
...
pdf(file = "./plots/cibersort_opc90_results_pericyte.pdf",width = 8,height = 8,pointsize = 16,useDingbats = F)
pheatmap(mat = cibersort_opc90,
         color = brewer.pal(9,"Reds"),
         show_rownames = F)#,
         # annotation_row = ann,
         # annotation_colors = list(undefined = c("TRUE" = "#000000FF","FALSE" = "#00000000")))
grid.text(label = "90 dpi ten-cell samples",x = 0.95,y = 0.4,rot = 90)
dev.off()
...
# Agani without endothelial cells
source("./opc90/import_opc90.R")
source("./functions/signature_matrix_pericytes.R")
source("./functions/normalizeTPM.R")

signature <- signatureMatrix(geneList = opc90$rsem$symbol,removeNFO = T,removeEndothelial = T)
commonGenes <- intersect(opc90$tpm$symbol,signature$tpm$symbol)
...
write.table(x = opc90_mixture,file = "./temp/mixture_opc90_pericyte_noEndothelial.txt",quote = F,sep = "\t",row.names = F)
...
commonGenes <- intersect(opc90$tpm$symbol,sig_avg_tpm$symbol)

opc90$tpm <- normalizeTPM(rsem = opc90$rsem[match(commonGenes,opc90$rsem$symbol),],index_counts = 10:ncol(opc90$rsem))

write.table(x = opc90$tpm[,c(3,10:ncol(opc90$tpm))],file = "./temp/mixture_opc90.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)

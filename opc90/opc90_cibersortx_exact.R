...
common_genes <- intersect(opc90_tpm$symbol,sig_avg$symbol)

sig_avg <- sig_avg[match(common_genes,sig_avg$symbol),]
...
# results
x <- read.table(file = "~/Downloads/CIBERSORTx_Job6_Results-2.txt",header = T,sep = "\t")
row.names(x) <- x$Mixture
x <- x[,2:7]
png(filename = "./plots/cibersort_opc90_absolute.png",width = 1200,height = 1200,res = 200)
pheatmap(x,col = (brewer.pal(9,"Reds")))
dev.off()
png(filename = "./plots/cibersort_opc90_absolute_saturate.png",width = 1200,height = 1200,res = 200)
pheatmap(x,col = (brewer.pal(9,"Reds")),breaks = seq(0,40,length.out=10))
dev.off()

...
wt90 <- bulk$tpm[,c(1:9,9+which(bulk$info$day == 90 & bulk$info$genotype == "WT"))]
wt150 <- bulk$tpm[,c(1:9,9+which(bulk$info$day == 150 & bulk$info$genotype == "WT"))]

wt12 <- wt12[,c(3,10:ncol(wt12))]
wt90 <- wt90[,c(3,10:ncol(wt90))]
wt150 <- wt150[,c(3,10:ncol(wt150))]

write.table(x = wt12,file = "./temp/mixture_bulk_12_wt_pericyte.txt",quote = F,sep = "\t",row.names = F)
write.table(x = wt90,file = "./temp/mixture_bulk_90_wt_pericyte.txt",quote = F,sep = "\t",row.names = F)
write.table(x = wt150,file = "./temp/mixture_bulk_150_wt_pericyte.txt",quote = F,sep = "\t",row.names = F)

# Run cibersort, get results, and place in external folder

f.cibersort_12_wt <- "./external/CIBERSORTx_bulk_12_wt.txt"
f.cibersort_90_wt <- "./external/CIBERSORTx_bulk90_pericyte.txt"
f.cibersort_150_wt <- "./external/CIBERSORTx_bulk_150_wt.txt"

cibersort_12_wt <- read.table(file = f.cibersort_12_wt,header = T,sep = "\t")
cibersort_90_wt <- read.table(file = f.cibersort_90_wt,header = T,sep = "\t")
cibersort_150_wt <- read.table(file = f.cibersort_150_wt,header = T,sep = "\t")

row.names(cibersort_12_wt) <- cibersort_12_wt$Mixture
row.names(cibersort_90_wt) <- cibersort_90_wt$Mixture
...
cibersort_90_wt <- cibersort_90_wt[,2:(which(names(cibersort_90_wt) == "P.value") - 1)] / cibersort_90_wt$Absolute.score..sig.score.
...
pdf(file = "./plots/bulk_LCM_purity_90_WT_cibersort_pericyte.pdf",width = 6,height = 6)
par(mar = c(1.8,4,1,8),mgp = c(2.9,1,0),xpd = T)
barplot(t(as.matrix(cibersort_90_wt)),ylab = "Fraction",names.arg = rep(x = "",nrow(cibersort_90_wt)),col = rev(brewer.pal(7,"Set1")),las = 1)
title(xlab = "90 dpi WT bulk samples",mgp = c(0.5,0,0))
legend(x = "topright",legend = names(cibersort_90_wt),pch = 15,col = rev(brewer.pal(7,"Set1")),inset = c(-0.4,0))
dev.off()
...
pdf(file = "./plots/cibersort_WT_90_boxplot_pericyte.pdf",width = 2.5,height = 2,pointsize = 7,useDingbats = F)
boxplot(x = cibersort_90_wt[,rev(c("OPC","pericyte","Astrocyte","Neuron","MO","Endothelial","Microglia"))],
        horizontal = T,las = 1,xlab = "Relative proportion",
        frame = F,xaxs = "i",yaxs = "i",axes = F,
        ylim = c(0,0.8),xlim = c(0.7,7.3),par(mar = c(3.1,4.6,0.5,1),mgp = c(2,1,0)),lwd = 0.5)
axis(side = 1,at = seq(0,0.8,0.2),lwd = 0.5)
axis(side = 2,at = seq(0,7)+0.5,labels = F,lwd = 0.5)
axis(side = 2,at = seq(1,7),labels = rev(c("OPC","pericyte","Astrocyte","Neuron","MO","Endothelial","Microglia")),las = 1,tick = F,lwd = 0.5,par(mgp = c(0,0.2,0)))
dev.off()

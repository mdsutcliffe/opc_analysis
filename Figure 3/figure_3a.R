...
pdf(file = "./Figure 3/figure_3a.pdf",width = 3.125,height = 3.125,pointsize = 7,useDingbats = F,bg = "white")
par(mai = c(0.75,0.5,0.25,0.5),mgp = c(1.6,0.6,0),xpd = T,lwd = 0.5/0.75)
plot(x = bulk$pc$projection1[bulk$info$day == 12],
     y = bulk$pc$projection2[bulk$info$day == 12],
     pch = ifelse(bulk$info$genotype[bulk$info$day == 12] == "WT",1,16),
     xlim = c(-60,60),ylim = c(0,140),
     frame = F,
     xaxs = "i",yaxs = "i",
     xlab = NA,ylab = "Proliferation",main = "Bulk 12 dpi samples",font.main = 1,cex.main = 8/7,
     axes = F)

...
write.table(x = sig_matrix,file = "./temp/signature_matrix_norm.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)

sig_matrix_colNorm <- sig_matrix
sig_matrix_colNorm[,2:ncol(sig_matrix_colNorm)] <- sig_matrix_colNorm[,2:ncol(sig_matrix_colNorm)] / colSums(sig_matrix_colNorm[,2:ncol(sig_matrix_colNorm)])
pheatmap(sig_matrix_colNorm[,2:ncol(sig_matrix_colNorm)],cluster_rows = F,cluster_cols = F,scale = "row")
write.table(x = sig_matrix_colNorm,file = "./temp/signature_matrix_colnorm.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)

sig_matrix_rownorm <- sig_matrix
sig_matrix_rownorm[1:nrow(sig_matrix_rownorm),2:ncol(sig_matrix_rownorm)] <- sig_matrix_rownorm[1:nrow(sig_matrix_rownorm),2:ncol(sig_matrix_rownorm)] / rowSums(sig_matrix_rownorm[1:nrow(sig_matrix_rownorm),2:ncol(sig_matrix_rownorm)])
pheatmap(sig_matrix_rownorm[,2:ncol(sig_matrix_rownorm)],cluster_rows = F,cluster_cols = F,scale = "row")
write.table(x = sig_matrix_rownorm,file = "./temp/signature_matrix_rownorm.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)
...
sig_genes <- sapply(X = 2:ncol(sig_avg_tpm),FUN = function(i) {
  ...
  ratio <- iGroup / iOther
  ratioOrder <- order(ratio,decreasing = T)
  sig_avg_tpm$symbol[ratioOrder[iGroup[ratioOrder] > 100][1:20]]
})
...
write.table(x = sig_matrix_pericyte,file = "./temp/signature_matrix_pericyte_20.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)
...
sig_genes <- sapply(X = 2:ncol(sig_avg_tpm),FUN = function(i) {
  ...
  ratio <- iGroup / iOther
  ratioOrder <- order(ratio,decreasing = T)
  sig_avg_tpm$symbol[ratioOrder[iGroup[ratioOrder] > 100][1:100]]
})
...
write.table(x = sig_matrix_pericyte_noEndothelial,file = "./temp/signature_matrix_pericyte_noEndothelial_100.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)
